<!-- filename: 2026-08-12-feedback-distortion-in-analog-circuits.md -->
***
layout: post
...
*Distortion and Feedback in Analog Circuits*
...
\eqref{eq:closed-loop-gain} is exact, but it is only useful if we know what $$ a $$ is — and in that derivation we quietly assumed $$ a $$ was some fixed constant, independent of signal level.
...
<!-- > **Author Note:** The source notes reference a figure showing the non-ideal open-loop transfer characteristic (with regions of slope $$ a_1 $$, $$ a_2 $$, and $$ a_3 = 0 $$), and later reference a second, distinct figure showing the transfer characteristic *after* feedback is applied. Both are marked with the same placeholder image tag in the original document, so I cannot tell from the notes alone whether these were meant to be the same figure or two separate figures. I have treated them as two separate figures below and flagged both as missing in the image manifest — please confirm and supply the correct image(s). -->

<!-- Author Note: Insert the referenced figure here (non-ideal open-loop amplifier transfer characteristic). -->
{% include figure.liquid path="assets/img/posts/feedback-distortion-in-analog-circuits/open-loop-transfer-characteristic.png" title="Non-ideal open-loop amplifier transfer characteristic with regions of slope a1, a2, and a3=0" class="img-fluid rounded z-depth-1" zoomable=true %}
...
<!-- Author Note: Insert the referenced figure here (closed-loop transfer characteristic after feedback is applied). -->
{% include figure.liquid path="assets/img/posts/feedback-distortion-in-analog-circuits/closed-loop-transfer-characteristic.png" title="Closed-loop transfer characteristic after feedback is applied, compared to the open-loop case" class="img-fluid rounded z-depth-1" zoomable=true %}
...
assets/img/posts/feedback-distortion-in-analog-circuits/

Required files:
1. Source/role: Non-ideal open-loop amplifier transfer characteristic, showing regions of slope a1, a2, and a3 = 0, used in "Where Nonlinearity Comes From"
   Exact filename: open-loop-transfer-characteristic.png
   Exact repository path: assets/img/posts/feedback-distortion-in-analog-circuits/open-loop-transfer-characteristic.png
   Used in post as: {% include figure.liquid path="assets/img/posts/feedback-distortion-in-analog-circuits/open-loop-transfer-characteristic.png" title="Non-ideal open-loop amplifier transfer characteristic with regions of slope a1, a2, and a3=0" class="img-fluid rounded z-depth-1" zoomable=true %}
   Status: missing — author must provide (source notes referenced this figure but did not include the actual image file; also flagged as ambiguous relative to figure 2 below)

2. Source/role: Closed-loop transfer characteristic after feedback is applied, used in "How Feedback Desensitizes the Gain"
   Exact filename: closed-loop-transfer-characteristic.png
   Exact repository path: assets/img/posts/feedback-distortion-in-analog-circuits/closed-loop-transfer-characteristic.png
   Used in post as: {% include figure.liquid path="assets/img/posts/feedback-distortion-in-analog-circuits/closed-loop-transfer-characteristic.png" title="Closed-loop transfer characteristic after feedback is applied, compared to the open-loop case" class="img-fluid rounded z-depth-1" zoomable=true %}
   Status: missing — author must provide (source notes referenced this figure with the same placeholder tag as figure 1; confirm this is a distinct image before deployment)
...
- Resolve the ambiguity flagged in items 1 and 2: the source document used the same image placeholder for what appear to be two different figures (open-loop vs. closed-loop transfer characteristics). Verify against the original notes/screenshots before finalizing filenames.
-->